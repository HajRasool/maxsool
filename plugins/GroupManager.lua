--Begin GroupManager.lua By @Maxsool
...
local function delmsg (MaTaDoR,Maxsool)
    msgs = MaTaDoR.msgs 
    for k,v in pairs(Maxsool.messages_) do
        msgs = msgs - 1
        tdcli.deleteMessages(v.chat_id_,{[0] = v.id_}, dl_cb, cmd)
        if msgs == 1 then
            tdcli.deleteMessages(Maxsool.messages_[0].chat_id_,{[0] = Maxsool.messages_[0].id_}, dl_cb, cmd)
            return false
        end
    end
    tdcli.getChatHistory(Maxsool.messages_[0].chat_id_, Maxsool.messages_[0].id_,0 , 100, delmsg, {msgs=msgs})
end
...
if not lang then
 return "》*#lø¢κ* _ρøšт łš αlяeαđч lø¢κeđ_🔒\n*〰〰〰〰〰〰〰〰*\n*øяđeявч*: [@"..check_markdown(msg.from.username or "Maxsool").."]"
elseif lang then
 return "》ارسال #لینک در گروه  از قبلا ممنوع است🔒\n*〰〰〰〰〰〰〰〰*\nسفارش توسط : [@"..check_markdown(msg.from.username or "Maxsool").."]"
end
else
